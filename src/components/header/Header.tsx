        ...
        <div className="flex items-center justify-between w-full gap-2 px-3 py-3 border-b border-gray-200 dark:border-gray-800 lg:border-b-0 lg:px-0 lg:py-4">
          ...
          <div className="hidden lg:block relative" ref={searchRef}>
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Pretraži lekove ili artikle..."
                value={query}
                onChange={(e) => handleSearchInput(e.target.value)}
                className="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-200 
                bg-transparent py-2.5 pl-12 pr-14 text-sm text-gray-800 shadow-theme-xs 
              ...
              <button className="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs text-gray-500 dark:text-gray-400 border border-gray-300 dark:border-gray-700 px-2 py-1 rounded">
                ⌘K
              </button>
            </div>

            {/* 🔥 SEARCH DROPDOWN — bez navigacije */}
            {showDropdown && searchResults.length > 0 && (
              <div className="absolute left-0 right-0 mt-2 bg-white dark:bg-gray-800 
              ...
              max-h-80 overflow-y-auto z-50">
                ...
                {searchResults.map((item) => (
                  <div
                    ...
                    hover:bg-gray-100 dark:hover:bg-gray-700 transition cursor-default"
                  >
                    {/* Naziv */}
                    ...
                    {item.type === "medicine" && (
                      <div className="text-xs mt-1 text-gray-600 dark:text-gray-400">
                        🏠 Dom: <span className="font-semibold">{item.quantity?.toFixed(2)}</span>  
                        <br />
                        👨‍👩‍👦 Porodica: <span className="font-semibold">{item.familyQuantity?.toFixed(2)}</span>
                      </div>
                    )}
                    ...
                    {item.type === "article" && (
                      <div className="text-xs mt-1 text-gray-600 dark:text-gray-400">
                        Stanje: <span className="font-semibold">{item.quantity}</span> kom
                      </div>
                    )}
                  </div>
                ))}
